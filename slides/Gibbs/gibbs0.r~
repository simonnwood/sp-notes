...
ps <- TRUE
if (ps) postscript("check-prior.eps",width=9,height=2)
...
## trace plots
if (ps) postscript("nh-trace.eps",width=9,height=5)
...
## acfs
if (ps) postscript("nh-acf.eps",width=9,height=3)
par(mfrow=c(1,2))
acf(th[1,])
acf(th[2,])
if (ps) dev.off()
...
if (ps) postscript("nh-popr.eps",width=9,height=3)
par(mfrow=c(1,2))
